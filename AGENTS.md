...
## Recent session changes (Sep 10, 2026 (5) — Sesión se cierra al cerrar el navegador: liveness del heartbeat + revocación central)

**Contexto**: pedido del usuario — "si se cierra el navegador la sesión de la persona debe cerrarse, no es propio de la seguridad requerida". Auditoría del ciclo de vida de la sesión concluyó que las cookies eran correctas (`CookieAuthSupport` sirve `aircargo_at`/`aircargo_rt` como **cookies de sesión, `SESSION_MAX_AGE=-1`**), pero quedaban 3 huecos reales: (1) el access token es stateless 15 min (`app.jwt.expiration-ms=900000`) y el refresh 7 días — un navegador con "continuar donde lo dejé" restaura las cookies de sesión y el usuario reaparece logueado; (2) el listado "Conectados" mantenía al usuario 300 s tras cerrar el navegador (sin purge inmediato ni beacon); (3) no había vinculación server-side entre la sesión y un navegador vivo. El refresh no se rechazaba nunca por liveness. Diagnóstico verificado en código: `AuthController.login/refresh/withCookies` (cookies de sesión), `ActiveSessionTracker` (heartbeat map, timeout 300s), `client.js` (401 → refresh por cookie → fallo → `clearLocalSession()` → `/login`), `router` (guard solo por localStorage), `TokenRevocationService` (bump de `tokens_valid_from`, memoizado 30s), `JwtUtil` (access 15 min / refresh 7 días).

**Fix — liveness del heartbeat como único test "navegador vivo"** (sin beacon de logout en `pagehide`: dispararía en F5/navegación y rompería multi-tab; la pestaña en background sigue latiendo y mantiene la sesión, así que liveness es seguro y multi-tab). El frontend late cada ~30s vía `GET /api/auth/heartbeat` mientras la app está abierta; si el navegador se cierra, los heartbeats se detienen → la sesión muere sola.

| File | Change |
|------|--------|
| `backend/.../authservice/service/ActiveSessionTracker.java` | Ventanas separadas y configurables: **`LIVENESS_SECONDS=120`** (fresco para refresh y lista de conectados) y **`REVOKE_TIMEOUT_SECONDS=300`** (umbral de revocación, conservador contra pestañas ocultas throttled). Nuevos métodos: **`isAlive(UUID)`** (heartbeat en los últimos 120s), **`staleUserIds()`** (sin heartbeat >300s, no destructivo) y **`purgeStaleSessions()`** ahora devuelve los userId removidos. `getConnectedUsers()` usa la ventana de 120s (el listado "Conectados" deja de mostrar al que cerró el navegador en ≤2 min). Constructor por defecto + **constructor de test con ventanas cortas** (sin dormir 120/300s en tests) |
| `backend/.../authservice/service/SessionRevocationGuard.java` | **NEW** — `@Scheduled(fixedRate=60_000)`: para cada `sessionTracker.staleUserIds()` hace `tokenRevocationService.bump(userId)` (adelanta `tokens_valid_from`) y luego `purgeStaleSessions()`. Efecto: **~5 min después de cerrar el navegador TODOS los tokens del usuario (access y refresh, incluidas cookies restauradas) quedan revocados de forma central** en todos los servicios (el `JwtAuthFilter` con `tokens_valid_from` los rechaza por request) |
| `backend/.../authservice/controller/AuthController.java` | **FIX** — `POST /refresh` añade parámetro `HttpServletResponse` y un **gate de liveness**: si `!sessionTracker.isAlive(user.getId())` → `CookieAuthSupport.clear(servletResponse, cookieSecure)` (borra cookies) + **401** `{"error":"Sesión cerrada: navegador cerrado"}`. Un navegador restaurado no puede refrescar la sesión si los heartbeats llevan >120s detenidos → el cliente (401 → refresh falla → `clearLocalSession()` → `/login`) fuerza el login real |
| `backend/.../authservice/src/test/.../ActiveSessionTrackerTest.java` | **NEW** — 5 tests con ventanas de 1−2s: heartbeat mantiene viva, sin heartbeat no viva, heartbeat viejo expira, listado de conectados con heartbeat fresco, y **cierre de navegador → stale → purge elimina** (simula el flujo completo del guard) |

**Decisiones de diseño**: (1) dos umbrales, no uno: el refresh es estricto (2 min) pero la revocación total es conservadora (5 min) para no revocar a pestañas ocultas que Chrome throttlea (con heartbeat ~1/min cualquier pestaña abierta mantiene viva la sesión; la inactividad del usuario NO marca, solo la ausencia de heartbeat); (2) sin beacon `pagehide`/`beforeunload` — se dispara también en F5 y navegación interna, y el Art. de limpieza multi-tab lo rompe; (3) el gate vive en `/refresh`, no en cada servicio: así el access cookie en navegador restaurado muere por la revocación central (~5 min de ventana máxima residual) y el refresh por liveness; (4) `bump()` es idempotente y cachea 30s, safe bajo reintentos del guard; (5) login/set-password/MFA no se tocan: cualquier login real crea sesión nueva con heartbeat inmediato (login ya hace `recordHeartbeat` en `LoginCommandHandler`). No hay tests previos que llamen `/refresh` (grep confirmado) → sin regresiones.

**Verificación**: `mvn -o test -pl aircargo-auth-service -am` con `JAVA_HOME=/usr/lib/jvm/java-21-amazon-corretto` → **BUILD SUCCESS** (common **22** + auth **41** = 63; `ActiveSessionTrackerTest` 5/5). Sin cambios de frontend (el cliente ya redirige a `/login` cuando el refresh falla). Falta opcional: deploy en EC2 (reiniciar auth-service con el jar nuevo) — no desplegado, sin commit.

## Recent session changes (Sep 10, 2026 (4) — Contenido móvil siempre alcanzable: overflow visible en móvil + filtros que envuelven + toolbar de matriz mawbs)

**Contexto**: pedido del usuario — "todavia quedan detalles en la vista de mawbs/ para el tab de matrix, en uld/ tamtien en las demas vistas". Diagnóstico por CDP móvil (innerWidth 500, harness `/tmp/opencode/cdp-chains.mjs` y `cdp-overflow.mjs`, token HS512 minted + cookies httpOnly + localStorage): la sonda de cadenas demostró que **TODAS las tablas ya scrollean bien** (dashboard flights `table-scroll-wrapper` clientW 470/scrollW 880; matriz mawbs 981; users 700; settings 1054) — tenían su propio contenedor `overflow-x`. El problema real era la **capa `.ds-page`**: `@apply overflow-hidden` (main.css L173-183) y en móvil seguía `hidden` → cualquier barra de herramientas/barra de filtros más ancha que el viewport quedaba **recortada e inalcanzable** (el `<main>` no generaba scroll horizontal). Caso estrella: la barra superior del tab **matriz** de mawbs medía **954px** (chips de estado + slider de fechas + botones Etiquetas/Export CSV/ocultar-pasadas) dentro de un contenedor de 472px con `overflow-x: visible` y SIN scroll → inaccesible. Además los `FilterBar` (bookings/flights~910-925px, ulds 513px) llevaban `shrink-0` en su raíz: al ser ítem de flex, se dimensionaba a su max-content (una sola línea) y **nunca envolvían**, por mucho `flex-wrap` que tuvieran.

| File | Change |
|------|--------|
| `frontend/src/assets/main.css` | **Fix sistémico**: dentro del bloque móvil `@media (max-width:640px)`, justo tras el colapso `.max-h-screen`, se añadió `.ds-page { overflow-x: visible !important; overflow-y: visible !important; }` (con comentario explicativo). Así NINGÚN toolbar/contenido ancho queda inalcanzable en móvil — `<main class="flex-1 overflow-auto">` pasa a ser el scroller que lo descubre. Las grillas app-shell (`ds-page-grid`, Bookings/Flights) conservan su `overflow:hidden !important` declarado más abajo en la hoja (scroll interno propio). Las tablas siguen aisladas en sus `.table-scroll-wrapper`. |
| `frontend/src/components/FilterBar.vue` | Raíz `flex flex-wrap items-end gap-2 shrink-0` → **`flex flex-wrap items-end gap-2 min-w-0`** (quitado `shrink-0`, añadido `min-w-0`). El ítem ahora puede encogerse al ancho disponible y entonces SUS hijos hacen wrap en varias líneas → la barra entera cabe en el viewport. Aplica a todas las vistas que usan FilterBar (bookings, flights, ulds, dashboard, exports, mawbs estados). |
| `frontend/src/views/MawbsView.vue` | Barra derecha del tab matriz (L47) `flex items-center gap-2` → **`flex flex-wrap items-center gap-2`** (chips + botones fluyen a su propia línea cuando faltan 500px). Slider de fechas (L70) `flex items-center overflow-x-auto scrollbar-none` (max-width:480px) → **`flex min-w-0 items-center overflow-x-auto scrollbar-none`** (sin `flex-1` para no alterar desktop; `min-w-0` permite encoger y su propio `overflow-x` interno scrollea el rango de meses en una línea propia). |

**Decisiones de diseño**: (1) el override `overflow-x/y: visible` es por-vista-móvil global (cambia la postura de `.ds-page` en móvil, corrigiendo la decisión anterior de "no tocar"): es correcto porque el catálogo de vistas no-app-shell (Dashboard/Mawbs/Receipts/Exports/…) usa scroll natural de página, y el `<main>` ya es `overflow-auto`; (2) `FilterBar` pierde `shrink-0` en TODOS los usos (se verifica que verticalmente crece y envuelve, nunca se aprieta); (3) el `flex-1` del slider se descartó para no empujar el select de vuelo en desktop — solo `min-w-0`.

**Verificación determinista (CDP, `cdp-overflow.mjs`/`cdp-chains.mjs` a innerWidth 500)**: antes → mawbs toolbar 954px con `parentOx:visible` y sin scroll (`mawbsTop` no vacío); bookings/flights filter bars 918/911px y ulds 513px con raíz `shrink-0` → clipeadas. Después → **las 10 rutas sin scroll horizontal de página** (`docScrollW=500`, `hasPageHScroll:false`) y cada elemento ancho vive dentro de su propio scroller `overflow-x` (header grids bookings 860 / flights 960, matriz mawbs 981, filas receipts 1000, lp-grid 960, tablas users 700 / settings 1054) → **todo alcanzable**. `mawbsTop` queda vacío (toolbar envuelto). La etiqueta residual "ASI" en ulds era un popup transitorio (no found en re-inspección; mainSW=500). Checks: `check:refs` ✓ 43 SFC 0 refs, `lint` ✓, `vitest` **18/18** ✓, `npm run build` ✓ (2.48s, `index-DTTnRoYA.js` / `MawbsView-chwFavR2.js`). Commit `247f542` → origin/main, rsync dist a EC2 (nginx HTTP 200, hash `index-DTTnRoYA.js` local==remoto). Perfil CDP y Chrome limpiados.

## Recent session changes (Sep 10, 2026 (3) — Tablas de dashboard/mawbs que no se veían en móvil: min-width para scroll horizontal)

**Contexto**: pedido del usuario — "Todavia falta mejorar mas el visual de la vista para moviles, hay tablas que no logran verse en dashboards/, en mawbs/ tenemos vistas que no se visualizan, mejoremos esto. En otras palabras arregla este sancocho". Diagnóstico por CDP móvil (innerWidth 500, harness `/tmp/opencode/cdp-mobile-tables.mjs`, token HS512 minted + cookies httpOnly + localStorage auth): de los 4 `<table>` de datos, 2 eran `<table class="w-full">` SIN `min-width` → el ancho lo fijaba el contenido intrínseco y quedaban aplastadas: la tabla **Weight Report** del Dashboard (11 columnas → scrollW **836px** en un cliente de 470px, columnas ~76px ilegibles) y la tabla **Lbs por Vuelo** de Mawbs (11 columnas → scrollW **615px**). El resto ya estaba bien: flights del Dashboard usa `minWidth` calculado (`tableMinWidth`), la matriz Mawbs usa anchos por columna (`colStyle`), y **Estados** ya tenía `min-width:1100px` (patrón a replicar).

| File | Change |
|------|--------|
| `frontend/src/views/DashboardView.vue` | Tabla Weight Report (L346): añadido `style="min-width: 1100px"` — mismo patrón que la tabla Estados de Mawbs (1100px para las 11 columnas awb/shipper/consignee/dest/commodity/flight/date/pcsRec/physical/dispatched/pcsDisp). El wrapper ya era `overflow-auto` → scroll horizontal cómodo en móvil; desktop intacto (1100 < ancho disponible). |
| `frontend/src/views/MawbsView.vue` | Tabla Lbs por Vuelo (L659): añadido `style="min-width: 1100px"` sobre el wrapper con `max-height:60vh; overflow-auto` existente (sigue proyectando la cabecera `thead sticky`). |

**Decisión de diseño**: (1) replicar el patrón ya usado por la tabla Estados (`min-width` inline estático de 1100px) en lugar de un override global en `main.css` — el resto de grillas ya resuelve su ancho propio (Bookings/Flights grids `min-width` por vista, matriz Mawbs por columna, flights dashboard calculado); (2) NO se tocó el layout app-shell (`ds-page-grid`) de estas vistas: siguen con scroll natural de página en móvil, solo se garantiza que las tablas sean legibles y desplacen horizontalmente; (3) ambas tablas tienen exactamente el mismo esquema de 11 columnas, así que comparten el mismo ancho mínimo.

**Verificación determinista (CDP, `cdp-mobile-tables.mjs`)**: antes → dashboard WR `tableW=836 / clientW=470`, mawbs lbs-vuelo `tableW=615 / clientW=470`; después → **ambas `tableW=scrollW=1100`** con `clientW=470` → scroll horizontal activo (header+filas juntas, sin truncar). Matriz mawbs intacta (981px por anchos de columna propios). Checks: `check:refs` ✓ 43 SFC 0 refs, `lint` ✓, `vitest` **18/18** ✓, `npm run build` ✓ (2.62s, `DashboardView-D8b3NdVG.js` / `MawbsView-BDhfg0Gl.js`). Commit `fa0cf35` → origin/main, rsync dist a EC2 (nginx HTTP 200, hash `index-CbLAJDnX.js` local==remoto). Perfil CDP y Chrome limpiados.
